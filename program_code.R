...
train_no_optimize_prediction_name <- paste(input2,'train_no_optimize_prediction.RData',sep='/')
LC_reversal_pair_name <- paste(input1,'LC_reversal_pair.RData',sep='/')

load(train_no_optimize_prediction_name)
load(LC_reversal_pair_name)
...
classify_result <- classification(reversal_pair = list(CRC_GC_pair=CRC_GC_pair),Beta = train_beta[,CRC_and_GC_predict_index],reversal_pair_cancer = 'CRC')
prediction_label[CRC_and_GC_predict_index] <- classify_result$prediction_label
table(train_annotation$Annotation,prediction_label)
rm(list=ls())
...
classify_result <- classification(reversal_pair = list(CRC_GC_pair=CRC_GC_pair),Beta = test_beta[,CRC_or_GC_predict_index],reversal_pair_cancer = 'CRC')
prediction_label[CRC_or_GC_predict_index] <- classify_result$prediction_label
table(test_annotation$Annotation,prediction_label)